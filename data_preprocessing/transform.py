...
if __name__ == '__main__':
    ...
    # choose one month data
    print('Total lines before filtered:')
    one_month_data = split_dataset.split('2013-01-07', '2013-01-31', line_id=route_short_name)
    trajectories_filtered = split_route(one_month_data, route_short_name)
    process.dump_json(trajectories_filtered, 'one_month_trajectories.json')
    process.dump_json(root_stop, 'one_month_start_stops.json')
    for start_stop, trajectory in trajectories_filtered.items():
        print('From {}, the number of trajectories:'.format(start_stop), len(trajectory))
