...
def main():
    ...
    dataset = ["Nov2012", "Jan2013"]
    for date in dataset:
        gps_root_path = home_dir + "gps/{}/".format(date)
        ...
        for gps_file in os.listdir(gps_root_path):
            gps_date = gps_file.split(".")[1]
            gps_path = gps_root_path + gps_file
            gps_date_dir = gps_trips_dir + gps_date + "/"
            if not os.path.exists(gps_date_dir):
                os .makedirs(gps_date_dir)
                time_1 = time.time()
                ...
                write_data(gps_trips, gps_date_dir)
                time_4 = time.time()
