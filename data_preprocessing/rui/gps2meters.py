...
def nearest_meters_gps(gps_trips_dir, def_trips, gps_meters_trips_dir):
    with os.scandir(gps_trips_dir) as entries:
        for entry in entries:
            ...
            if entry.is_file() and entry_name.endswith(".csv"):
                ...
                gps_meters_trip = [["seq_id", "dist_travelled", "cum_time", "np_time"]]
                def_trip_file = entry_name.split("_")[2]
                print("def_trip_file:", def_trip_file)
                for i in def_trips[def_trip_file]:
                    seq_id = i / 100
                    cum_distance = i
                    _, ind = kd_tree.query((0, cum_distance))
                    nearest_point = locations[ind]
                    cum_time, speed = gps_trips[nearest_point]
                    del_time = (cum_distance - nearest_point[1]) / speed
                    if del_time < 0:
                    ...
                    if cum_time > max_time:
                        # print('error:', cum_time, max_time)
                        cum_time = max_time
                    new_row = [seq_id, cum_distance, cum_time, start_time]
                    gps_meters_trip.append(new_row)
                    # del locations[:ind]  # re-construct kd-tree to ensure the timestamp is not decreasing
                    kd_tree = spatial.cKDTree(locations)

                with open(gps_meters_trips_dir + entry_name, "w", encoding='utf-8') as csv_out:
...
def main():
    ...
    gps_meters_trips_dir = home_dir + "processed/gps_meters_trips/" + bus_line_number + "/"
    ...
    for gps_trips_day_dir in os.listdir(gps_trips_dir):
        ...
        nearest_meters_gps(gps_trips_dir + gps_trips_day_dir + "/", def_trips, gps_meters_trips_dir)
